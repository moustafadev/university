...
class NumberWithSize extends StatefulWidget {
  const NumberWithSize({super.key});

  @override
  State<NumberWithSize> createState() => _PictogramState();
}

class _PictogramState extends State<NumberWithSize> {
  bool isShow = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("арабское число с размером",
          style: TextStyle(
            ...
            Column(
              ...
              children: numberRandom
                ...
                child: Text(
                  e,
                  style: TextStyle(
                      fontSize: (8 * (int.tryParse(e)??0)).toDouble(), fontWeight: FontWeight.bold),
                ),
              ))
                  .toList(),
            ),
